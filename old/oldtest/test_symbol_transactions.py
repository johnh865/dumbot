...
from dumbot.dataclasses import Action, SymbolTransactions
...
def test_3():
    ...
    actions = [a1, a2, a3]
    sh = SymbolTransactions(symbol=symbol, actions=actions)
...
def test_exec_one():
    ...
    actions2 = [b1, b2, b3]
    sh = SymbolTransactions(symbol=symbol, actions=[])    
    sh.add(b1)
    sh.add(b2)
    sh.add(b3)
...
def test_interday():
    ...
    a3 = Action(date=date3, symbol=symbol, name=ACTION_SELL_PERCENT, amount=1)     
    a4 = Action(date=date3, symbol=symbol, name=ACTION_BUY, amount=1) 
