...
class Strat1(Strategy):
    ...
    def init(self):
        ...
        self.growth = Indicators(
            STOCKS, 
            fn_growth, 
            fargs=(self.window,)).normalize_by_all
        
        self.volat = Indicators(
            STOCKS,
            fn_std_change,
            fargs=(self.window,)).normalize_by_each
    ...
    def next(self):
        ...
        growth = self.growth.array_before(self.date)[-1]
        volat = self.volat.array_before(self.date)[-1]
        
        metrics = growth - np.maximum(volat - 0.5, 0) 
...
if __name__ == '__main__':
    ...
    windows = np.arange(10, 400, )
    windows = [optimal_window]
    performances = []
